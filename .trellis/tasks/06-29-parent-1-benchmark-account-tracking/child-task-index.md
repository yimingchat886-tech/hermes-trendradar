...
| 8 | `06-30-child-8-mediacrawler-collection-runner` | Final external-runtime child: MediaCrawler plus openai-whisper local deployment through CLI/process adapters. | child 2, child 3, child 4; run last | Local external runtime evidence, importability proof, transcript/log retention, cleanup proof. |

## Execution Rule

Start with child 1. Child 8 runs last and merges the earlier MediaCrawler/Whisper runtime scopes. It keeps dry-run/fake-command mode, then performs the real local deployment smoke test with the user-provided temporary cookie, 1 account, and 1 public video.

## Scope Guard

RSSHub, TrendRadar, hotspot radar, formal RAG vector storage, release review, autonomous topic-title generation, credential storage, proxy setup, platform bypass guidance, queue/concurrency/daemon mode, and long-term video storage are not part of this child set.
