...
- Child 6a media manifest: `/home/jym/workspace/_external/hermes-stock-runs/run-child6a-media-download-manifest-20260703-local/media-manifest.private.jsonl`.
- Child 6 external run root: `/home/jym/workspace/_external/hermes-stock-runs/run-child6-transcription-threshold-smoke-20260703-local`.
- Child 6 fresh SQLite DB: `/home/jym/workspace/_external/hermes-stock-runs/run-child6-transcription-threshold-smoke-20260703-local/child6-transcription-threshold.sqlite3`.
- User PLAN confirmation: `执行child task 6` on 2026-07-03.
...
5. Prepare local-only runtime inputs:
   ...
   - fresh isolated SQLite DB under that run root, or an approved copied state DB with verification scoped to this Child 6 run id, selected 99 contents, model profile, and artifact namespace;
...
- None. Execution completed; see `stage-report.md`.

## Confirmation Gate

- [x] User confirmed this PLAN via `执行child task 6`.
- [x] Oracle high-risk PLAN re-review completed for the pinned 99-item revision via `child6-transcript-threshold-plan-rereview-5`.
